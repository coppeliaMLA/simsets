...
action_viewing = np.transpose(
    np.array(
        [[np.random.binomial(1, x * y, 1).item() for x in action] for y in action_rep]
    )
)
...
sci_fi_viewing = np.transpose(
    np.array(
        [[np.random.binomial(1, x * y, 1).item() for x in sci_fi] for y in sci_fi_rep]
    )
)
...
# Convert to json
viewing_json = viewing_df.to_dict(orient="records")
